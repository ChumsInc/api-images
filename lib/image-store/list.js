const debug = require('debug')('chums:lib:image-store:list');
const formidable = require('formidable');
const sharp = require('sharp');
const fs = require('fs').promises;
const path = require('path');
const {addImagePath, loadProductImages, removeImagePath} = require('./db-handler');
const {IMG_SIZES, IMAGE_PATHNAMES, IMAGE_PATHS, imgPath} = require('./settings');
const {makeProductImage} = require('./utils');
...
exports.getAllImages = async (req, res) => {
    try {
        ...
        const images = await loadProductImages(props);
...
async function syncDirectory({pathname, rebuild}) {
    try {
        ...
        let imageList = await loadProductImages();
        ...
        imageList = await loadProductImages();
