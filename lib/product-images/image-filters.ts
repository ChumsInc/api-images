...
export async function loadWarehouse(): Promise<Warehouse[]> {
    ...
    try {
        const [rows] = await mysql2Pool.query<WarehouseRecord[]>(sql);
        return rows;
    } catch (err: unknown) {
...
export async function loadProductLines(): Promise<ProductLine[]> {
    ...
    try {
        const [rows] = await mysql2Pool.query<ProductLineRecord[]>(sql);
        return rows.map(row => ({
            ...row,
            active: row.active === 1,
        }))
...
export async function loadCategory2(): Promise<ProductCategory[]> {
    try {
        const sql = `SELECT DISTINCT i.Category2,
                     ...
                     ORDER BY Category2`;
        const [rows] = await mysql2Pool.query<CategoryRecord[]>(sql);
        return rows.map(row => ({
            ...row,
...
export async function loadCategory3(): Promise<ProductCollection[]> {
    ...
    try {
        const [rows] = await mysql2Pool.query<CollectionRecord[]>(query);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
...
export async function loadCategory4(): Promise<BaseSKU[]> {
    const query = `SELECT DISTINCT i.Category4,
                   ...
                   ORDER BY Category4`;
    try {
        const [rows] = await mysql2Pool.query<BaseSKURecord[]>(query);
        return rows.map(row => ({
            ...row,
...
export async function loadCountryOfOrigin(): Promise<CountryOfOrigin[]> {
    try {
        ...
        const [rows] = await mysql2Pool.query<CountryOfOriginRecord[]>(query);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
...
export async function loadPrimaryVendor(): Promise<PrimaryVendor[]> {
    try {
        ...
        const [rows] = await mysql2Pool.query<PrimaryVendorRecord[]>(query);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
 ...
 */
export async function loadProductStatusList(): Promise<ProductStatus[]> {
    const query = `SELECT id, code, description
                   FROM c2.IM_ItemStatus
                   ORDER BY code`;
    try {
        const [rows] = await mysql2Pool.query<ProductStatusRecord[]>(query);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
...
export async function getItemFilterOptions(req: Request, res: Response): Promise<void> {
    try {
        const [
            warehouses,
            productLines,
