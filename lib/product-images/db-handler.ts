...
export async function loadImagesByItemList(itemCodes: string[] = []): Promise<ItemImageRecord[]> {
    try {
        ...
        const [_rows] = await mysql2Pool.query<ImageLookupRow[]>(sql1, {itemCodes});
...
export async function loadImages({
                                     ...
                                     activeImages,
                                     preferred
                                 }: LoadImagesProps = {}): Promise<ProductImage[]> {
    try {
        const query = `
            SELECT filename,
                   pathnames,
        ...
        const [[images], altItems] = await Promise.all([
            mysql2Pool.query<ProductImageRow[]>(query, data),
 ...
 */
export async function saveImage(props: ProductImageProps): Promise<ProductImage> {
    try {
...
export async function setImageActive(filename: string, active: string): Promise<ProductImage> {
    try {
...
export async function setPreferredImage(filename: string, itemCode: string): Promise<void> {
    try {
...
export async function loadAltItemCodes(filename: string): Promise<ProductAltItem[]> {
    try {
        ...
        const args = {filename};
        const [rows] = await mysql2Pool.query<ProductAltItemRow[]>(sql, args);
        return rows.map(row => ({
            ...row,
            active: !!row.active,
        }))
...
export interface AddImagePathProps {
    filename: string;
    pathname: string;
...
export async function addImagePath({
                                       ...
                                       img_format,
                                       color_space
                                   }: AddImagePathProps): Promise<ProductImage | null> {
    try {
